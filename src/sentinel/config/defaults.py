...
_P_COMPETITOR_SYNTH = (
    ...
    "findings:\n\n{public_findings}\n\n"
    "Rules:\n"
    "1. Each item in strengths/weaknesses/pricing_signals/recent_developments is a Finding with "
    "two fields: `text` (one concrete sentence) and `source` (an object with `boundary='public'`, "
    "`label` = source name, and `url` = the full HTTP URL from your research — never null for public findings).\n"
    "2. Populate `how_to_win` with concrete counter-positioning angles.\n"
    "3. If a category had no reliable source, add a Gap rather than inventing content.\n"
    "4. Set the `target` field to the competitor name."
)
_P_CLIENT_PLANNER = (
...
_P_CLIENT_SYNTH = (
    ...
    "{private_note}"
    "\n\nMerge them into Findings. Each Finding has two fields: `text` (one concrete sentence) "
    "and `source` (an object with `boundary`, `label`, and `url` — populate `url` with the full "
    "HTTP URL whenever a public web source is referenced). "
    "'public_signal' Findings MUST have source.boundary='public' and include source.url. "
...
_P_COMPETITOR_SYNTH_2T = (
    ...
    "1. Each item in strengths/weaknesses/pricing_signals/recent_developments is a Finding with "
    "two fields: `text` (one concrete sentence stating the finding) and `source` (an object with "
    "`boundary='public'`, `label` = the source name, and `url` = the full HTTP URL from the "
    "extraction — populate `url` whenever the extraction includes a URL, never leave it null for "
    "public web sources).\n"
    ...
    "WHY it is sparse in the finding `text` (e.g. 'No public pricing page found') — the gap is "
    "itself a competitive signal.\n"
...
_P_CLIENT_SYNTH_2T = (
    ...
    "\n\nMerge them into Findings. Each Finding has two fields: `text` (one concrete sentence) "
    "and `source` (an object with `boundary`, `label`, and `url` — populate `url` with the full "
    "HTTP URL from the extraction whenever a public web source is referenced). "
    "'public_signal' Findings MUST have source.boundary='public' and include source.url. "
    "'private_signal' Findings MUST have source.boundary='private'. The 'merged_insights' "
    "field is the core value — state what the COMBINATION of public + private implies "
    "(e.g. 'public hiring surge + stalled deal stage → re-engage with expansion offer'). "
...
_P_PLANNER = (
    ...
    "- CRITICAL — `compare` dependency rule: a `compare` step MUST depend_on EXACTLY "
    "[one self_profile step, one competitor step]. NEVER have a compare step that depends on "
    "two competitor steps — that is invalid and will crash at runtime. If the objective asks "
    "to compare two external companies with no 'us' side, still include one `self_profile` "
    "step as the reference anchor.\n"
    "- A 'market-capture strategy' / overall recommendation uses one `program_strategy` step that "
    "depends_on the `compare` step(s).\n"
    "- Canonical example — objective 'Compare Perplexity vs BiltIQ': [self_profile (us)] + "
    "[competitor (Perplexity)] + [competitor (BiltIQ)] → [compare (depends on self_profile + "
    "Perplexity)] + [compare (depends on self_profile + BiltIQ)] → optionally [program_strategy]. "
    "NEVER: [competitor (Perplexity)] + [competitor (BiltIQ)] → [compare (both competitors)] — "
...
_P_FINANCE_SYNTH = (
    ...
    "• key_metrics: top 6 most important cited financial figures (revenue, profit, margin, valuation). "
    "Each entry: metric_name, value (with units), period, source (boundary='public', label, url).\n"
    "• market_position: 2 sentences on competitive standing and key rivals.\n"
    "• risk_signals: top 5 risks. Each entry: metric_name=risk name, value=brief description (max 20 words), source.\n"
    "• recent_developments: top 5 material events. Each entry: metric_name=event name, value=brief description (max 20 words), source.\n"
    "• financial_summary: 1 paragraph narrative (max 120 words) covering growth, profitability, and key risks.\n"
    "• investment_thesis (optional): 1-2 sentences neutral bull/bear — facts only, no advice.\n\n"
    "CONCISENESS: limit each list field to 6 items max. Keep each Finding.text under 25 words. "
...
_P_FINANCE_SYNTH_2T = (
    ...
    "• target, one_line_summary (max 20 words)\n"
    "• key_metrics: top 6 cited figures — metric_name, value (with units), period, source (boundary='public').\n"
    "• market_position: 2 sentences on competitive standing and peers.\n"
    "• risk_signals: top 5 risks — metric_name=risk, value=brief description (max 20 words), source.\n"
    "• recent_developments: top 5 events — metric_name=event, value=brief description (max 20 words), source.\n"
    "• financial_summary: 1 paragraph narrative (max 120 words).\n"
    "• investment_thesis (optional): 1-2 sentences neutral facts-only synthesis.\n\n"
    "CONCISENESS: limit each list field to 6 items max. Keep each Finding.text under 25 words. "
    "Rules: Every Finding.source.boundary MUST be 'public'. "
...
_P_PRODUCT_RESEARCH_PUBLIC = (
    ...
    "Research plan:\n{research_plan}\n\n"
    "HARD LIMIT: maximum 12 total tool calls (find_deals + firecrawl_search combined). "
    "After 12 calls, stop immediately and compile all findings gathered so far. Do NOT exceed 12.\n\n"
    "ATTACK SEQUENCE — run these in order:\n"
    "1. PRICES FIRST (1 call) — call `find_deals` once with '<product_type> <key_spec>'. It runs "
    "`google_shopping_search` for live priced listings, then `google_product` to expand the top "
    "model into per-seller offers. Authoritative source for current prices; each result carries "
    "title, price (₹), source_url, seller.\n"
    "2. REVIEWS (3-4 calls) — search '<product_type> <key_spec> review 91mobiles OR notebookcheck OR digit.in' "
    "for expert scores and pros/cons per model. Max 4 review searches total.\n"
    "3. COMPARISON (2-3 calls) — search '<product_type> <key_spec> price comparison India' for articles "
    "listing multiple models side-by-side. Max 3 comparison searches.\n"
    "4. FALLBACK (max 4 calls) — ONLY if find_deals returned nothing: search "
    "'site:flipkart.com <product_type> <key_spec> buy price' and "
    "'site:amazon.in <product_type> <key_spec> buy price'.\n\n"
    "STOP when you hit 12 total calls regardless of step. "
    ...
    "Group findings by product model. Include AT LEAST 5 qualifying models if found. "

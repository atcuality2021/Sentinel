...
// ---------------------------------------------------------------------------
// TerminalAnimation — simple scenarios adapter used by the task detail page
// Converts the { id, label, steps[] } API to the TabContent[] compound API.
// ---------------------------------------------------------------------------

export interface TerminalScenarioStep {
  command: string
  output?: string
}
...
export function TerminalAnimation({
  ...
  const tabs: TabContent[] = scenarios.map((s) => ({
    label: s.label,
    command: s.steps[0]?.command ?? "running…",
    lines: s.steps.slice(1).map((step) => ({
      text: step.output ? `${step.command}  →  ${step.output}` : step.command,
      color: step.output === "ERROR" ? "text-red-400" : "text-neutral-300",
    })),
  }))

  return (
    <TerminalAnimationRoot
      tabs={tabs.length ? tabs : [{ label: "log", command: "waiting…", lines: [] }]}
